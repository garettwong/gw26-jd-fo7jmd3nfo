...
YEAR = 2026
BUILD_ID = "v04-change-markers-comparison-filter-20260714-v05c"
CONTEXT_SRC = OUTDIR / "class_context.json"
OVERRIDES_SRC = OUTDIR / "schedule_overrides.json"
COMPARE_BASELINE = OUTDIR / "versions" / "2026-07-14-R04"
COMPARE_LABEL = "V04"
...
.filter-jump-target{scroll-margin-top:12px}.top-option{width:48px;height:44px;border:0;border-radius:8px;background:#fff;color:#0f7074;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1px;cursor:pointer;font-family:inherit}.top-option:hover{background:#eef7f7}.top-option:focus-visible{outline:3px solid #ffc857;outline-offset:1px}.top-option .mode-main{font-size:18px;line-height:.85}.top-option .mode-sub{font-size:7px;font-weight:850;line-height:1}.floating-mode-switch{grid-template-columns:repeat(5,48px)}@media (max-width:820px){.floating-mode-switch{grid-template-columns:repeat(5,42px)}.top-option{width:42px;height:42px}}@media (min-width:821px) and (max-width:1400px){.floating-mode-switch{grid-template-columns:repeat(3,36px)}.today-option{grid-column:1/span 2;width:75px;height:28px}.top-option{grid-column:3;width:36px;height:28px}.top-option .mode-main{font-size:13px}.top-option .mode-sub{font-size:5.5px}}@media (orientation:landscape) and (max-height:540px){.floating-mode-switch{grid-template-columns:repeat(5,34px)}.top-option{width:34px;height:32px;border-radius:6px}.top-option .mode-main{font-size:12px}.top-option .mode-sub{font-size:5px}}
'''
...
function openChip(el){{
  ...
  modal.querySelector('.modal-body').innerHTML='<span class="modal-current-label">Current V05c content</span><div class="modal-current">'+(html||txt)+'</div>'+comparison+(source?'<div class="modal-source">Source: '+esc(source)+'</div>':'');
  modal.hidden=false;
}}
