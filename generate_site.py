...
YEAR = 2026
BUILD_ID = "v20j-hk244-sitin-hk239-judy-teachers-20260810a"
CONTEXT_SRC = OUTDIR / "class_context.json"
OVERRIDES_SRC = OUTDIR / "schedule_overrides.json"
VERSIONS_SRC = OUTDIR / "versions.json"
COMPARE_BASELINE = OUTDIR / "versions" / "2026-08-06-V20i"
COMPARE_LABEL = "V20j"
COMPARE_BASELINE_LABEL = "V20i"
...
CLASS_UPDATE_META = {
    ...
    "HK244HG · CW8": ("2026-08-10", "最新導師表：8 月 14 日 L3 由 Calvin sit-in 代上；8 月 31 日 L7 由 Chan Shuk Ki 任教；9 月 4、7、8 日 L10–L12 由 Calvin 任教；9 月 8 日期末實務試 14:30–17:30。"),
    "HK239HG · FS": ("2026-08-10", "最新導師表：Garett 任教 L1–L4，包括 8 月 14 日下午 L2；8 月 21 日 L5 由 Judy 任教，L6 由 Judy 任教；期末考試 15:30–16:30。"),
...
def lifecycle_status(group_events):
    ...
    future_events = [
        ...
        and not re.search(r"\b(?:cancelled|canceled)\b|取消", event["text"], re.I)
    ]
    if any(
...
.summary-class-dates{display:flex;grid-column:1/-1;align-items:flex-start;gap:5px;min-width:0;padding-top:6px;border-top:1px solid #c9dbdc}.summary-class-dates-label{flex:0 0 auto;padding-top:3px;color:#526174;font-size:8px;font-weight:950}.summary-class-date-list{display:flex;flex-wrap:wrap;gap:4px;min-width:0}.summary-class-date{display:inline-flex;align-items:center;justify-content:center;min-height:20px;padding:2px 5px;border:1px solid #9eacbb;border-radius:4px;background:#fff;color:#526174;font-size:8px;font-weight:900;line-height:1;white-space:nowrap}.summary-class-date.mine{border:2px solid #063f43;background:#0f7074;color:#fff;box-shadow:0 0 0 1px #fff inset}.class-summary-card.unconfirmed .summary-class-dates{border-top-color:#d8b985}.class-summary-card.active .summary-class-date.mine{background:#0f7074;color:#fff}.class-summary-key .my-date-key i{border:2px solid #063f43;background:#0f7074}.class-summary-key .other-date-key i{border:1px solid #9eacbb;background:#fff}
.summary-update{display:grid;grid-column:1/-1;grid-template-columns:auto minmax(0,1fr);gap:6px;align-items:start;padding-top:6px;border-top:1px solid #c9dbdc;color:#405064;font-size:9px;font-weight:750;line-height:1.3}.summary-update strong{color:#0f7074;font-size:9px;font-weight:950;white-space:nowrap}.class-summary-card.unconfirmed .summary-update{border-top-color:#d8b985}.completed .summary-update strong{color:#526174}
...
<label class="lesson-log-field full"><span>今堂教了甚麼</span><textarea id="lessonLogTaught" autocomplete="off"></textarea></label>
<label class="lesson-log-field"><span>進度／未完成</span><textarea id="lessonLogProgress" autocomplete="off"></textarea></label>
...
(function(){
  ...
  const inputs={
    taught:document.getElementById('lessonLogTaught'),
    progress:document.getElementById('lessonLogProgress'),
    follow_up:document.getElementById('lessonLogFollowUp'),
    remarks:document.getElementById('lessonLogRemarks')
  };
  ...
  let active=null;
  let store={schema_version:1,updated_at:null,notes:{}};
  ...
  const mergeStores=(left,right)=>{
    const merged={schema_version:1,updated_at:new Date().toISOString(),notes:{}};
  ...
  const updateBadges=()=>{
    document.querySelectorAll('.lesson-log-open').forEach(button=>{
      const exists=Boolean(store.notes[button.dataset.logKey]);
  ...
  form.addEventListener('submit',async event=>{
    ...
    store.notes[active.key]=Object.assign({},active,{taught:inputs.taught.value.trim(),progress:inputs.progress.value.trim(),follow_up:inputs.follow_up.value.trim(),remarks:inputs.remarks.value.trim(),updated_at:now,source_build:window.ERB_BUILD_ID});
    store.updated_at=now;saveLocal();updateBadges();
    if(!config.token){setStatus('已儲存在本機；設定 token 後才會跨裝置同步。','ok');return;}
...
def lesson_log_trigger(ev):
    if event_layer(ev) != "mine" or ev.get("category") in {"holiday", "mike"}:
        return "", ""
    if CANCELLED_TEACHING_RE.search(str(ev.get("text") or "")):
        return "", ""
    fields = event_fields(ev)
    key = "|".join([
        str(ev.get("date") or ""),
        str(ev.get("group_label") or ""),
        str(fields.get("lesson") or ""),
    ...
    button = (
        ...
        f'data-log-lesson="{ehtml(fields.get("lesson", ""))}" '
        f'data-log-time="{ehtml(fields.get("time", ""))}" '
        f'aria-label="Record what was taught for this lesson">記錄</span>'
    )
    return " has-lesson-log", button
...
def class_summary_button(group, section):
    ...
    all_class_dates = sorted({event["date"] for event in all_group_events})
    mine_class_dates = {event["date"] for event in mine_events}
    class_dates_html = ''.join(
        f'<span class="summary-class-date{" mine" if date_value in mine_class_dates else ""}" '
        f'title="{"你需要任教" if date_value in mine_class_dates else "其他導師"}">'
        f'{calendar.month_abbr[datetime.date.fromisoformat(date_value).month]} '
        f'{datetime.date.fromisoformat(date_value).day}</span>'
        for date_value in all_class_dates
    )
    if section == "upcoming":
    ...
    update_date, update_note = CLASS_UPDATE_META.get(label, ("未記錄", "尚未建立改動摘要。"))
    html_button = (
        ...
        f'<span class="summary-class-dates"><span class="summary-class-dates-label">全班日期</span>'
        f'<span class="summary-class-date-list">{class_dates_html}</span></span>'
        f'<span class="summary-update"><strong>最後更新 {ehtml(update_date)}</strong>'
        f'<span>{ehtml(update_note)}</span></span>'
        f'</button>'
    )
...
def class_summary_section(section, heading, heading_id):
    ...
    status_key = (
        ...
        '<span><i></i>Confirmed</span><span class="pending"><i></i>Unconfirmed</span>'
        '<span class="my-date-key"><i></i>你任教日期</span>'
        '<span class="other-date-key"><i></i>其他導師日期</span></div>'
...
const transitMinutes={{
  ...
  'ymca_yau_ma_tei|wan_chai':40,'wan_chai|ymca_yau_ma_tei':40,
