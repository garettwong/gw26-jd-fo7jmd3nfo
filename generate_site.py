...
SRC = Path(r"C:/Users/garet/OneDrive/桌面/Timetable/ERB Super Timetable 04_checking 02.xlsx")
...
def chip(ev):
    ...
    return (f'<div class="chip {st} cat-{ev["category"]}" tabindex="0" role="button" '
            ...
            f'<div class="ttl">{ehtml(ev["title"])}</div><div class="det">{ehtml(ev["detail"])}</div></div>')
...
def month_html(year, month):
    ...
    for week in cal.monthdatescalendar(year, month):
        for day in week:
            ...
            cells.append(f'<div class="{cls}" id="d-{ds}"><div class="dnum">{day.day}</div>' + ''.join(chip(e) for e in evs) + '</div>')
    ...
    agenda = ''.join(f'<div class="aday"><div class="adate"><span class="adow">{day.strftime("%a")}</span><span class="anum">{day.day}</span></div><div class="achips">' + ''.join(chip(e) for e in by_date[day.isoformat()]) + '</div></div>' for day in daykeys)
...
cat_filters = ''.join(f'<button class="filter" data-filter="{ehtml(cat)}">{ehtml(label)} ({sum(1 for e in events if e["category"] == cat)})</button>' for cat, label in CATS)
...
<div class="section-h">Filter</div><div class="filters"><button class="filter active" data-filter="all">All</button>{cat_filters}</div>
{months_html}
...
document.querySelectorAll('.filter').forEach(btn=>btn.addEventListener('click',()=>{{
  ...
  const f=btn.dataset.filter; document.querySelectorAll('.chip').forEach(ch=>{{ ch.style.display=(f==='all'||ch.classList.contains('cat-'+f))?'':'none'; }});
...
(function(){{
 const now=new Date(); const ds=now.toISOString().slice(0,10); let target=document.getElementById('d-'+ds);
 if(target){{target.classList.add('today'); target.id='today'; setTimeout(()=>target.scrollIntoView({{block:'center',inline:'center'}}),250);}}
 else{{document.getElementById('todayBtn').href='#m6';}}
}})();
</script></body></html>'''
