...
def month_html(year, month):
    ...
    for day in daykeys:
        ds = day.isoformat()
        agenda_bits.append(f'<div class="aday" id="a-d-{ds}" data-date="{ds}"><div class="adate"><span class="adow">{day.strftime("%a")}</span><span class="anum">{day.day}</span></div><div class="achips">' + ''.join(chip(e) for e in by_date[ds]) + '</div></div>')
...
(function(){{
 ...
 todayBtn.addEventListener('click', e=>{{e.preventDefault(); focusToday();}});
 focusToday();
