...
YEAR = 2026
BUILD_ID = "coursefilter-blackframes-20260708b"
...
GROUPS = [(label, _group_slugs[label]) for label in _group_labels]
...
cat_filters = ''.join(f'<button class="filter" data-filter="{ehtml(slug)}">{ehtml(label)} ({sum(1 for e in events if e["group"] == slug)})</button>' for label, slug in GROUPS)
...
<meta name="erb-build" content="coursefilter-blackframes-20260708b">
...
document.querySelectorAll('.filter').forEach(btn=>btn.addEventListener('click',()=>{{
  ...
  const f=btn.dataset.filter; document.querySelectorAll('.chip').forEach(ch=>{{ ch.style.display=(f==='all'||ch.classList.contains('grp-'+f))?'':'none'; }});
