  ...
  try{
    ...
    function renderExpected(schedule,remainingOnly=false){
      ...
      const expectedHtml=groups.map(group=>{
        ...
        const rows=group.rows.map(r=>'<tr class="'+(r.received?'received-payment':'')+'"><td><span class="pay-kind '+(r.kind==='SEN'?'sen':'')+'">'+esc(r.kind)+'</span><span class="pay-label">'+esc(r.label)+'</span>'+(r.received?'<span class="received-badge">已收款</span>':'')+'<span class="pay-course">'+esc(r.course_name)+'</span></td><td>'+esc(r.invoice_date)+'</td><td class="pay-date">'+esc(r.expected_payment_date)+'</td><td><span class="pay-basis">'+esc(r.basis)+'</span><br>'+hours(r.hours)+' hours</td><td class="total">'+money(r.amount)+'</td></tr>').join('');
