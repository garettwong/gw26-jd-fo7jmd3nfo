...
def version_build_id(item: dict) -> str:
    version_index = ROOT / "versions" / item["id"] / "index.html"
...
def version_row(item: dict) -> str:
    ...
    build_id = html.escape(version_build_id(item), quote=True)
    build_query = f"&amp;build={build_id}" if build_id else ""
    return f'''<a class="version" href="../versions/{version_id}/?v=redtext1{build_query}">
...
document.querySelectorAll('.version').forEach(link=>link.addEventListener('click',event=>{{
  if(event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;
  event.preventDefault();
  document.querySelectorAll('.version').forEach(item=>item.classList.remove('opening'));
  link.classList.add('opening');
  link.setAttribute('aria-busy','true');
  link.querySelector('.open').textContent='Opening';
